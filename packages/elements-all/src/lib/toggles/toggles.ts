...
export const diagonalInfo: ElementInfo<schema.DiagonalElement['value']> = {
    ...
    menu: {
        ...
        name: 'Diagonals',
        checkbox: [
            { name: 'Positive Diagonal', icon: 'positive-diagonal', refPath: 'positive' },
            { name: 'Negative Diagonal', icon: 'negative-diagonal', refPath: 'negative' },
        ],
        icon: 'positive-diagonal',
    },
    getWarnings(
        value: schema.DiagonalElement['value'],
        grid: Grid,
...
export const disjointGroupsInfo: ElementInfo<schema.BooleanElement['value']> = {
    ...
    menu: {
        ...
        checkbox: { name: 'Disjoint Groups', icon: 'disjoint' },
        icon: 'disjoint',
    },
    getWarnings(
        value: schema.BooleanElement['value'],
        grid: Grid,
        _regionMap: IdxMap<Geometry.CELL, number>,
        digits: IdxMap<Geometry.CELL, number>,
        warnings: IdxBitset<Geometry.CELL>,
        ...
        if (value) {
            for (const [pos] of product(grid.width)) {
                ...
                for (const [bx] of product(grid.width)) {
                    cells.push(
                        cellCoord2CellIdx(
                            [Math.floor(bx / 3) * 3 + Math.floor(pos / 3), (bx % 3) * 3 + (pos % 3)],
                            grid,
                        ),
                    );
                }
...
export const consecutiveInfo: ElementInfo<schema.ConsecutiveElement['value']> = {
    ...
    menu: {
        ...
        name: 'Nonconsecutive',
        checkbox: [
            { name: 'Nonconsecutive Orthogonal Neighbors', icon: 'consec-orth', refPath: 'orth' },
            { name: 'Nonconsecutive Diagonal Neighbors', icon: 'consec-diag', refPath: 'diag' },
        ],
        icon: 'consec-orth',
    },
    getWarnings(
        value: schema.ConsecutiveElement['value'],
        grid: Grid,
        _regionMap: IdxMap<Geometry.CELL, number>,
        digits: IdxMap<Geometry.CELL, number>,
        warnings: IdxBitset<Geometry.CELL>,
