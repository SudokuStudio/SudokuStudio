...
type WatchEvent = { path: string[]; oldVal: Data; newVal: Data };
type Log = WatchEvent[];

function loggingWatcher<T extends Data>(log: Log): Watcher<T> {
...
describe('class StateManager', () => {
    describe('update & watch', () => {
        it('basic', () => {
            ...
            stateMgr.update({ foo: 'bar' });
            expect(log.length).toEqual(0);

            stateMgr.update({ name: 'jake' });
        ...
        it('wildcard', () => {
            const stateMgr = new StateManager();
            stateMgr.update({ people: { jack: { age: 5 } } });
        ...
        it('handles replacement (select)', () => {
            const stateMgr = new StateManager();
            stateMgr.update({ select: { 0: true, 1: true, 2: true, 3: true } });

            stateMgr.ref('select', '2').replace(null);
            expect(stateMgr.get()).toEqual({ select: { 0: true, 1: true, 3: true } });

            stateMgr.update({ select: { 4: true } });
    ...
    describe('delete', () => {
        it('basic delete', () => {
            ...
            stateMgr.update({ 'foo/bar/baz': 5 });
            expect(log.length).toEqual(1);
            expect(log[log.length - 1].path).toEqual(['foo']);
            expect(log[log.length - 1].oldVal).toBeNull();
            expect(log[log.length - 1].newVal).toEqual({ bar: { baz: 5 } });

            stateMgr.update({ foo: null });
            expect(log.length).toEqual(2);
            expect(log[log.length - 1].path).toEqual(['foo']);
        ...
        it('deeply-cascading delete', () => {
            ...
            stateMgr.update({ 'foo/bar/baz/qux': 5 });
            expect(log.length).toEqual(1);
        ...
        it('no-op deletes', () => {
            const stateMgr = new StateManager();
            stateMgr.update({ foo: { bar: 5 } });

            const log: Log = [];
            stateMgr.watch(loggingWatcher(log), false, 'foo');

            stateMgr.update({ 'foo/bar/baz/qux': {} });
            expect(log.length).toEqual(0);

            stateMgr.update({ 'foo/bar/baz/qux': null });
            expect(log.length).toEqual(0);

            stateMgr.update({ 'foo/bar/baz/qux': undefined });
            expect(log.length).toEqual(0);
    ...
    describe('triggerNow', () => {
        ...
        it('trigger on existing with wildcard', () => {
            const stateMgr = new StateManager();
            stateMgr.update({ ages: { jake: 50, jason: 52, morty: 14, mingwei: 23 } });
    ...
    describe('edge cases', () => {
        it('replace obj with true', () => {
            ...
            {
                const diff = stateMgr.update({ foo: { 0: 'a', 1: 'b', 2: 'c' } });
                expect(diff).toEqual({
                    redo: { 'foo/0': 'a', 'foo/1': 'b', 'foo/2': 'c' },
                    undo: {
                ...
                expect(stateMgr.get()).toEqual({ foo: { 0: 'a', 1: 'b', 2: 'c' } });
            }

            {
                const diff = stateMgr.update({ foo: true });
                expect(diff).toEqual({ redo: { foo: true }, undo: { 'foo/0': 'a', 'foo/1': 'b', 'foo/2': 'c' } });
                expect(stateMgr.get()).toEqual({ foo: true });
            }
        });
        ...
        it('handle false basic', () => {
            ...
            stateMgr.update({ foo: false });
            expect(stateMgr.get()).toEqual({ foo: false });
        ...
        it('handle false nested', () => {
            ...
            stateMgr.update({ foo: { bar: false } });
            expect(stateMgr.get()).toEqual({ foo: { bar: false } });
        ...
        it('handle bool object', () => {
            ...
            stateMgr.update({ foo: { positive: true, negative: false } });
            expect(stateMgr.get()).toEqual({ foo: { positive: true, negative: false } });

            expect(log.length).toEqual(1);
            expect(log[log.length - 1].path).toEqual(['foo']);
            expect(log[log.length - 1].oldVal).toBeNull();
            expect(log[log.length - 1].newVal).toEqual({ positive: true, negative: false });

            stateMgr.update({ foo: { positive: true, negative: true } });
            expect(stateMgr.get()).toEqual({ foo: { positive: true, negative: true } });

            expect(log.length).toEqual(2);
            expect(log[log.length - 1].path).toEqual(['foo']);
            expect(log[log.length - 1].oldVal).toEqual({ positive: true, negative: false });
            expect(log[log.length - 1].newVal).toEqual({ positive: true, negative: true });
        });
        ...
        it('overwrite primitive', () => {
            ...
            stateMgr.update({ foo: 5 });
            expect(log.length).toEqual(1);
            expect(log[log.length - 1].path).toEqual(['foo']);
            expect(log[log.length - 1].oldVal).toBeNull();
            expect(log[log.length - 1].newVal).toEqual(5);

            stateMgr.update({ 'foo/bar': 10 });
            expect(log.length).toEqual(2);
            expect(log[log.length - 1].path).toEqual(['foo']);
            expect(log[log.length - 1].oldVal).toEqual(5);
    ...
    describe('diff', () => {
        it('makes diffs for simple values', () => {
            ...
            {
                const diff = stateMgr.update({ a: 1 });
                expect(diff).toEqual({ redo: { a: 1 }, undo: { a: null } });
            }

            {
                const diff = stateMgr.update({ a: 2 });
                expect(diff).toEqual({ redo: { a: 2 }, undo: { a: 1 } });
            }
        });
        ...
        it('makes diffs nested', () => {
            ...
            let diff: null | Diff;
            diff = stateMgr.update({ foo: 'bar' });
            expect(diff).toEqual({ redo: { foo: 'bar' }, undo: { foo: null } });

            diff = stateMgr.update({ 'foo/baz': 'bar' });
            expect(diff).toEqual({ redo: { 'foo/baz': 'bar' }, undo: { foo: 'bar' } });

            diff = stateMgr.update({ 'foo/baz': 'bar' });
            expect(diff).toBeNull();

            diff = stateMgr.update({ foo: null });
            expect(diff).toEqual({ redo: { 'foo/baz': null }, undo: { 'foo/baz': 'bar' } });

            diff = stateMgr.update({ bang: null });
            expect(diff).toBeNull();
        });
        ...
        it('makes diffs for updates with multiple entries', () => {
            ...
            {
                const diff = stateMgr.update({ a: 1, b: 1, c: 1 });
                expect(diff).toEqual({ redo: { a: 1, b: 1, c: 1 }, undo: { a: null, b: null, c: null } });
            }

            {
                const diff = stateMgr.update({ b: 2, c: 2, d: 2 });
                expect(diff).toEqual({ redo: { b: 2, c: 2, d: 2 }, undo: { b: 1, c: 1, d: null } });
            }
        });
        ...
        it('val to empty obj', () => {
            ...
            {
                const diff = stateMgr.update({ foo: { bar: 5 } });
                expect(diff).toEqual({ redo: { 'foo/bar': 5 }, undo: { 'foo/bar': null } });
            }

            {
                const diff = stateMgr.update({ foo: { bar: {}, baz: 7 } });
                expect(diff).toEqual({ redo: { 'foo/baz': 7 }, undo: { 'foo/baz': null } });
            }
        });
        ...
        it('shrink diff', () => {
            ...
            {
                const diff = stateMgr.update({ foo: { bar: { baz: { n: 5 } } } });
                expect(diff).toEqual({ redo: { 'foo/bar/baz/n': 5 }, undo: { 'foo/bar/baz/n': null } });
            }

            {
                const diff = stateMgr.update({ foo: null });
                expect(diff).toEqual({ redo: { 'foo/bar/baz/n': null }, undo: { 'foo/bar/baz/n': 5 } });
            }
        });
        ...
        it('generated 1', () => {
...

            {
                ...
                expect(diff).toEqual({
                    redo: { 'orb/fao/arr/oar/oro': 231, 'for/bfb/rrf/rff': 0.108941585665983, 'oao/bfa': 'roobobf' },
                    undo: { 'orb/fao/arr/oar/oro': null, 'for/bfb/rrf/rff': null, 'oao/bfa': null },
                });
            }

            {
                const diff = stateMgr.update({ orb: {}, for: {}, oao: {} });
                expect(diff).toEqual({
                    redo: { 'orb/fao/arr/oar/oro': null, 'for/bfb/rrf/rff': null, 'oao/bfa': null },
                    undo: { 'orb/fao/arr/oar/oro': 231, 'for/bfb/rrf/rff': 0.108941585665983, 'oao/bfa': 'roobobf' },
                });
            }
        });
    });
