...
const objectFromEntries =
    'function' === typeof (Object as any).fromEntries
...
type WatcherTree = { [WatchersKey]?: Set<Watcher<any>>; [K: string]: WatcherTree };

type UpdateInternal = { data: Data; redo: [string, Data][]; undo: [string, Data][] };
...
export class StateRef {
    ...
    update(update: Update): null | Diff {
        ...
        const rootUpdate = objectFromEntries(Object.entries(update).map(([k, v]) => [path + k, v]));
        return this._stateManager.update(rootUpdate);
    }

    // https://svelte.dev/docs#Store_contract
    subscribe<T extends Data>(subscription: (value: T | null) => void): () => void {
        const watch = this.watch<T>((_path, _oldData, newData) => subscription(newData), true);
...
export class StateManager {
    ...
    update(update: Update): null | Diff {
        ...
        if (changed) {
            return { redo: objectFromEntries(redo) as Update, undo: objectFromEntries(undo) as Update };
        }
        return null;
    }
    ...
    private static _unwatchPattern(watcher: Watcher<any>, watcherTree: WatcherTree, segs: string[]): boolean {
        ...
        if ('.' === seg && !this._unwatchPattern(watcher, watcherTree, restSegs)) return false;
        if (!Object.prototype.hasOwnProperty.call(watcherTree, seg)) return false;
        if (!this._unwatchPattern(watcher, watcherTree[seg], restSegs)) return false;

        if (0 >= Object.keys(watcherTree[seg]).length && null == watcherTree[WatchersKey]) delete watcherTree[seg];
        return true;
    }
    ...
    private static _updateInternal(
        ...
        const {
            ...
            if (atUpdateDepth) {
                ...
                const allKeys = new Set([...Object.keys(dataObj || {}), ...Object.keys(updateObj || {})]);
                ...
                if (0 === Object.keys(newData).length) return { data: updateObj || update, redo, undo };
            ...
            const {
                ...
                redo,
                undo,
            } = StateManager._updateInternal(innerData, update, segsRest, [...path, key], nextWatcherTrees);
            if (isEq(innerData, newInnerData)) {
                // No changes.
                return { data, redo, undo };
            }
    ...
    private static _nextWatcherTrees(key: string, watcherTrees: WatcherTree[]): WatcherTree[] {
        ...
        for (const watcherTree of watcherTrees) {
            if (null != watcherTree['*']) {
                nextTrees.push(watcherTree['*']);
            }
            if (null != watcherTree[key]) {
                nextTrees.push(watcherTree[key]);
            }
        }
        return nextTrees;
    }
