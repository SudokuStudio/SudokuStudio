<script lang="ts">
    ...
    import type { CheckboxMenuInfo } from '$lib/js/element/element';
    ...
    export let info: CheckboxMenuInfo;
    ...
    function unused(data: unknown): boolean {
...
<ConstraintRow {id} {deletable} name={info.name} unused={unused($valueRef)} {onClick} onTrash={() => removeElement(id)}>
    {#each Array.isArray(info.checkbox) ? info.checkbox : [info.checkbox] as { name, icon, refPath } (refPath)}
