...
export function pushHistory(history: Diff | null): boolean {
    ...
    userState.update({ [`history/${Date.now()}`]: JSON.stringify(history), historyUndone: null });
    return true;
}
...
export function changeHistory(redo: boolean): boolean {
    ...
    if (null == histVal) {
        ...
        userState.update({ [`${redo ? 'historyUndone' : 'history'}/${histKey}`]: null });
        changeHistory(redo);
    }
...
if (browser) {
    ...
    (window as any).changeHistory = changeHistory; // TODO
