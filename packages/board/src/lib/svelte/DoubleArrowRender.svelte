<script lang="ts">
    ...
    function getItems(items: Record<string, ArrayObj<Idx<Geometry.CELL>>> | null): Item[] {
    ...
    const items = derived(ref, (items) => getItems(items as Record<string, ArrayObj<Idx<Geometry.CELL>>> | null));
</script>
