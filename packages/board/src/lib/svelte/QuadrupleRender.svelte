<script lang="ts">
    ...
    import type { ArrayObj, schema } from '@sudoku-studio/schema';
    ...
    function each(value: schema.QuadrupleElement['value'] | undefined): Item[] {
        const out: Item[] = [];
        for (const [cornerIdx, digits] of Object.entries(value || {})) {
            const [x, y] = cornerIdx2cornerCoord(+cornerIdx, grid);
            const digitStr = arrayObj2array(digits as ArrayObj<number>).join('');

            const atSide = x <= 0 ? -1 : grid.width <= x ? +1 : 0;
            const atBotTop = y <= 0 ? -1 : grid.height <= y ? +1 : 0;

            const lines: Item['lines'] = (digitStr.match(atSide ? splitterAtEdge : splitter) || []).map(
                (line, i, lines) => {
                    return {
                        lx: x,
            ...
            out.push({ idx: +cornerIdx, x, y, lines });
        }
        return out;
    }
</script>
...
<g {id}>
    {#each each($ref as schema.QuadrupleElement['value']) as { idx, x, y, lines } (idx)}
        <circle
            cx={x}
            cy={y}
            r={radius}
            ...
            clip-path="url(#quadruple-{id}-mask)"
        />
        {#each lines as { lx, ly, line, textAnchor, domBaseline } (`${lx},${ly}`)}
            <text
                x={lx}
                y={ly}
