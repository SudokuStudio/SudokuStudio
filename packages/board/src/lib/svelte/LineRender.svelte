<script lang="ts">
    ...
    function each(items: Record<string, ArrayObj<Idx<Geometry.CELL>>> | null): Item[] {
...
<g {id}>
    {#each each($ref as Record<string, ArrayObj<Idx<Geometry.CELL>>> | null) as { itemId, d } (itemId)}
        <path
            {d}
            fill="none"
