<script lang="ts">
    ...
    function getMarks(
        cells: IdxMap<Geometry.CELL, Record<string, number>> | null | undefined,
...
<g {id} mask="url(#SUDOKU_MASK_GIVENS_FILLED)">
    {#each getMarks($ref as IdxMap<Geometry.CELL, Record<string, number>> | null | undefined) as { idx, x, y, keys, nums } (idx)}
        <text
            {x}
            {y}
            text-anchor="middle"
            ...
            font-size="0.3"
            font-weight="600"
            textLength={keys.length > 5 ? '0.9' : undefined}
