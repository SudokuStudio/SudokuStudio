<script lang="ts">
    ...
    export let idx2coord: <TAG extends Geometry>(idx: Idx<TAG>, grid: Grid) => Coord<TAG>;
    ...
    function each(value: schema.SeriesNumberElement['value'] | null | undefined): Item[] {
        const out: Item[] = [];
...
<g {id}>
    {#each each($ref as schema.SeriesNumberElement['value'] | null | undefined) as { idx, x, y, text } (idx)}
        {#if 0 < radius}
