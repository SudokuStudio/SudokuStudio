...
>
    ...
    {#each each($ref as schema.KillerElement['value']) as { cageId, text, labelPos } (cageId)}
        <!-- Mask-out each sum label. -->
        <rect
            x={labelPos.x}
            y={labelPos.y}
            width={0.7 * `${text}`.length * fontSize}
            height={1.1 * fontSize}
            fill="#000"
        />
    {/each}
...
<g {id}>
    {#each each($ref as schema.KillerElement['value']) as { cageId, text, labelPos, d } (cageId)}
        <text
            x={labelPos.x}
            y={labelPos.y}
            text-anchor="start"
        ...
        <path
            ...
            stroke-dasharray="0.075 0.04"
            mask="url(#killer-{id}-mask)"
        />
    {/each}
</g>
