<script lang="ts">
    ...
    function each(value: schema.EdgeNumberElement['value']): Item[] {
        ...
        for (const [ edgeIdx, digitOrTrue ] of Object.entries(value)) {
            const text = true !== digitOrTrue ? `${digitOrTrue}` : '';
...
<g {id}>
    {#each each($ref || {}) as { idx, x, y, text } (idx)}
        ...
        <text {x} {y} text-anchor="middle" dominant-baseline="central" fill={textColor} font-size={fontSize} font-weight="600">{text}</text>
    {/each}
</g>
