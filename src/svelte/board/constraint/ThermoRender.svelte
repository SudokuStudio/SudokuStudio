...
<marker id="thermo-bulb-{id}"
    ...
    orient="auto"
>
    <circle cx="0.5" cy="0.5" r="0.4" fill="#aaa" />
</marker>
<g {id}>
    {#each Object.entries($ref) as [ thermoId, idxArr ] (thermoId)}
        <path d={makePath(idxArr)} fill="none" stroke="#aaa" stroke-width="0.2" marker-start="url(#thermo-bulb-{id})" stroke-linejoin="round" stroke-linecap="round" />
    {/each}
</g>
